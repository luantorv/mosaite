...
\textbf{Chat RAG}: Responde teoría contable y uso del sistema. Se accede desde el menú \textbf{Chat}.

\textbf{ConsultorIA}: Consulta los datos contables. Se activa escribiendo \texttt{/ia} antes de la pregunta en la barra de búsqueda superior.

\subsection{¿Cómo uso ConsultorIA?}

Escriba \texttt{/ia} seguido de su pregunta en la barra de búsqueda superior y presione Enter. Por ejemplo: \texttt{/ia ¿cuántas cuentas activas hay?}. El resultado aparece en el panel de búsqueda como una consulta SQL y una tabla de datos.

\subsection{¿El Chat crea asientos automáticamente?}

No. Es una herramienta de aprendizaje que responde preguntas, no crea transacciones. ConsultorIA tampoco modifica datos: solo realiza consultas de lectura.

\subsection{ConsultorIA no entiende mi pregunta}

Verifique que la pregunta empiece con \texttt{/ia}. Luego reformúlela de manera más simple y específica, usando nombres o códigos correctos de cuentas y especificando períodos claramente.
